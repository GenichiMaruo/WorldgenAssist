...
No speedup
or private-seed confidentiality is claimed. See [the evidence report](docs/TEST_RESULTS_LATEST.md)
and [multi-PC procedure](docs/MULTIPC_TESTING.md) for exact status and limits.
...
environment, never a public server. See `docs/SEED_CONFIDENTIALITY.md` for the
...
`docs/SEEDED_LEAF_FIXTURE_PROTOCOL.md` for the narrow exception and test runner.
...
application, local fallback and cleanup. See `docs/TEST_RESULTS_LATEST.md`,
`docs/SEEDED_LEAF_ORCHESTRATION.md` and `docs/TEST_HANDOFF.md` for exact evidence,
...
Start with [the repository instructions](docs/AGENTS.md), then read the
[architecture](docs/ARCHITECTURE.md),
[verified worldgen pipeline](docs/WORLDGEN_PIPELINE.md), and
[Mixin verification ledger](docs/MIXIN_TARGETS.md).
