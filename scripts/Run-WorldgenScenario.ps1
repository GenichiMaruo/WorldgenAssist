...
try {
    ...
    $scenarioSeconds=if($Purpose -eq 'performance'){900}else{420};$deadline=[DateTime]::UtcNow.AddSeconds($scenarioSeconds);while(-not $server.HasExited){if($tunnel.HasExited){throw 'SSH tunnel exited during scenario'};foreach($client in $clients){if($client.process.HasExited){throw "Client exited during scenario: $($client.name) exit_code=$($client.process.ExitCode)"}};if([DateTime]::UtcNow -gt $deadline){throw 'Scenario exceeded its bounded deadline'};Start-Sleep -Milliseconds 500}
